...
export default function ProductDetailClient({ product }: ProductDetailClientProps) {
  ...
  async function handleBuy() {
    if (product.isFree) {
      ...
      link.href = `/api/download?slug=${product.slug}`;
      link.download = "";
    ...
    try {
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({ mode: "payment", productSlug: product.slug }),
      });
      const data = await res.json();
      if (!res.ok) {
        if (res.status === 401) {
          router.push(`${lp}/auth/login?next=${lp}/products/${product.slug}`);
          return;
        }
        toast.error(data.error ?? t("buyError"));
        return;
      }
      if (data.url) {
      ...
      <div className="mt-10 lg:mt-0">
        <div className="sticky top-24 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6 shadow-sm">
          ...
          <button
            onClick={handleBuy}
            disabled={buyLoading}
            className={`w-full py-3.5 rounded-xl font-semibold text-base transition-all active:scale-95 mb-2 disabled:opacity-60 disabled:cursor-not-allowed ${
              product.isFree
