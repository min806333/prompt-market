...
function renderMarkdown(text: string) {
  return text.split("\n").map((line, i) => {
    ...
    return <p key={i} className="text-gray-600 dark:text-gray-400">{line.replace(/\*\*(.*?)\*\*/g, (_, m) => m)}</p>;
  });
}
      ...
      <Container size="md">
        ...
        <article>
          <div className="flex items-center gap-3 mb-4">
            ...
            {ann.is_pinned && <span className="text-xs text-gray-400">📌 고정</span>}
          </div>
          ...
          <div className="mt-12">
            ...
            <div className="space-y-2">
              {(others ?? []).map((o) => (
                <Link
                  key={o.id}
                  href={`/announcements/${o.slug ?? o.id}`}
