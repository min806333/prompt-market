...
export async function generateStaticParams() {
  return announcements.map((a) => ({ id: a.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { id } = await params;
  const ann = announcements.find((a) => a.slug === id);
  if (!ann) return {};
  return {
    title: `${ann.title} — 공지사항 | PromptMarket`,
  ...
  return text
    ...
    .map((line, i) => {
      if (line.startsWith("**") && line.endsWith("**")) {
        return <p key={i} className="font-bold text-gray-900 dark:text-white mt-4 mb-1">{line.replace(/\*\*/g, "")}</p>;
      }
      if (line.startsWith("- ")) {
        return <li key={i} className="ml-4 list-disc text-gray-600 dark:text-gray-400">{line.slice(2).replace(/\*\*(.*?)\*\*/g, "$1")}</li>;
      }
...
export default async function AnnouncementDetailPage({ params }: Props) {
  ...
  const others = announcements.filter((a) => a.id !== ann.id).slice(0, 3);
    ...
    <div className="py-12">
      <Container size="md">
        <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8">
          ...
          <span>/</span>
          <Link href="/announcements" className="hover:text-gray-600 dark:hover:text-gray-300">공지사항</Link>
          <span>/</span>
        ...
        <article>
          ...
          <p className="text-sm text-gray-400 mb-8">{formatDate(ann.createdAt)}</p>
        ...
        {others.length > 0 && (
          <div className="mt-12">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">다른 공지사항</h2>
            ...
            <div className="mt-4 text-center">
              <Link href="/announcements" className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
                공지사항 전체 보기 →
              </Link>
            </div>
          </div>
